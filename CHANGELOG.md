...
- Codex costs: stop sustained CPU use with large local histories. Refreshes and catch-up passes no longer decode the whole cost cache again, menu refreshes read only usage inside their date range, and a 30-day refresh no longer evicts history that a recent 365-day Usage & Spend scan still needs (#3882).
